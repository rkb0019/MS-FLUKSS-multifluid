! pure subroutine dfCubicSpline( NEND, X, Y, B, C, D, iBCBGN, iBCEND, IERR )
subroutine dfCubicSpline( NEND, X, Y, B, C, D, iBCBGN, iBCEND, IERR )

!                                                        == dfCubicSpline.f90 ==
!//////////////////////////////////////////////////////////////////////////////!
!==                                                                          ==!
!==  PURPOSE:                                                                ==!
!==    dfCubicSpline - interpolatory approximation by cubic splines with     ==!
!==  arbitrary end conditions                                                ==!
!==                                                                          ==!
!==  REFERENCES:                                                             ==!
!==                                                                          ==!
!==  ARGUMENTS:                                                              ==!
...
!  real(kind = 8),automatic :: an, bn, rn, h, h1, h2, DN, DNM1, DNM2

  integer        :: i, im, ip, NM1, NM2, NM3, bgnBC, endBC
  real(kind = 8) :: an, bn, rn, h, h1, h2, DN, DNM1, DNM2
...
!                                           Terminal - x not monotone increasing
  ...
  do   i = 2, NM1 
    ...
    ip     = i + 1
    h1     = C(ip)/D(im)
    B(i)   = 3.0D0*(C(ip)*D(i) + C(i)*D(ip)) - h1*B(im)
    D(i)   = 2.0D0*(C(ip)      + C(i)      ) - h1*C(im)
  enddo
