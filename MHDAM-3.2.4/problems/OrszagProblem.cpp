...
// Null constructor
OrszagProblem::OrszagProblem()
{
  m_isFortranCommonSet = false;
}

// Input parameters
void OrszagProblem::input( ParmParse & parser, int verbosity )
{
  ...
  if( verbosity >= 2 )
  {
    pout() << "Orzag-Tang vortex problem input:"      << endl;
...
void OrszagProblem::setFortranCommon( const Real&     a_gamma )
{
    CH_assert(m_isFortranCommonSet == false);

    FORT_SETORSZAG( CHF_CONST_REAL( a_gamma     ) );
...
void OrszagProblem::setFortranCommonSet()
{
  m_isFortranCommonSet = true;
}
...
PhysProblem* OrszagProblem::new_PhysProblem()
{
  OrszagProblem* retval = new OrszagProblem();
  ...
  retval->setSourceCalculator( getSourceCalculator() );

  return static_cast<PhysProblem*>(retval);
}

// Set boundary fluxes
void OrszagProblem::fluxBC(       FArrayBox&      a_F,
                                  FArrayBox&      a_Bn,
                            const FArrayBox&      a_W,
...
void OrszagProblem::initialize(LevelData<FArrayBox>& a_U)
{
  CH_assert(m_isFortranCommonSet == true);
  CH_assert(m_isDefined == true);
  ...
  for (dit.begin(); dit.ok(); ++dit)
  {
    ...
    FORT_ORSZAGINIT(CHF_CONST_FRA(U),
                    CHF_CONST_INT(iCP),
                    CHF_CONST_REAL(dx),
                    CHF_BOX(uBox));
  }
                                                             ...
                                                             // Fill ghost cells
void OrszagProblem::fillGhostCells(       FArrayBox&      a_W,
                                    const FArrayBox&      a_U,
                                    const int&            a_dir,
                                    const Real&           a_time)
{
...
void OrszagProblem::getBCFlags( eBoundaryConditions leftBC,
...
PhysProblem::eBoundaryConditions OrszagProblem::getBCFlags( int a_dir, Side::LoHiSide a_sd )
{
  if( a_dir == 2 )
    return BC_Undefined;
